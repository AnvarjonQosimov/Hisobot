...
// Since we used fragment, we need to change the final `</div>` to `</>`
...
code = code.replace(/<button[\s\S]*?className="mobile-menu-btn"[\s\S]*?<\/button>/m, '');
code = code.replace(/<button[\s\S]*?className="right-panel-toggle-btn"[\s\S]*?<\/button>/m, '');
...
// Strip out the logout dialog
code = code.replace(/\{logoutDialog && \([\s\S]*?className="confirm-overlay"[\s\S]*?<div className="confirm-buttons"[\s\S]*?<\/div>\s*<\/div>\s*<\/div>\s*\)\}/g, '');
...
console.log('ProjectReport.js rewritten correctly!');
